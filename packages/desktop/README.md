...
```bash
bun run dev          # 日常开发：daemon + web 联调
bun run desktop:dev  # 接近成品的桌面壳联调
```
...
```bash
COZYBASE_BUN_SOURCE=/path/to/custom/bun bun run desktop:prepare
```

若需要显式指定 target triple：

```bash
COZYBASE_BUN_TARGET=x86_64-apple-darwin bun run desktop:prepare
```
