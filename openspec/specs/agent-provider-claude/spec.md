...
定义 Claude Code provider 如何将 Claude Agent SDK 的原始消息流转换为标准化的 Agent 事件流，并管理查询生命周期。
...
- **WHEN** Claude SDK 发出 `{ type: 'user' }` 消息
...
Provider SHALL 通过追踪流式信号自动生成 `messageId`，在首次 delta 到来时 emit `conversation.message.started`，使调用方无需了解 Claude SDK 的 streaming 内部结构。
