...
- `req: Request | undefined` — 标准 Web Request 对象；当触发来源为 cron 时 MUST 为 `undefined`
...
#### Scenario: HTTP 触发时构建 FunctionContext

- **WHEN** Runtime 处理 HTTP 函数请求并准备执行函数
- **THEN** 系统 SHALL 构建 FunctionContext，包括 `req`（原始请求）、`db`（注册表条目的 DB 连接）、`env`（环境变量）、`app`（APP 名称和模式）、`mode`（stable/draft）、`trigger = 'http'`、`log`（Logger 实例）、`platform`（PlatformClient 实例）

#### Scenario: cron 触发时构建 FunctionContext

- **WHEN** Runtime 通过 schedule 执行函数
- **THEN** 系统 SHALL 构建 FunctionContext 且 `req = undefined`、`trigger = 'cron'`
