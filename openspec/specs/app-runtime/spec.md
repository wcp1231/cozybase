...
Runtime SHALL 为每个已加载的 APP 提供以下对外路由：

- `GET /apps/:name/` -- UI 首页
- `GET /apps/:name/assets/*` -- UI 静态资源
- `GET /apps/:name/ui.json` -- UI Schema
- `ALL /apps/:name/fn/:fnName` -- Functions 执行
- `GET /apps/:name/db/schema` -- DB Schema 查询
- `POST /apps/:name/db/_sql` -- Raw SQL 执行
- `GET /apps/:name/db/:table` -- 列表查询
- `POST /apps/:name/db/:table` -- 创建记录
- `GET /apps/:name/db/:table/:id` -- 获取记录
- `PATCH /apps/:name/db/:table/:id` -- 更新记录
- `DELETE /apps/:name/db/:table/:id` -- 删除记录
