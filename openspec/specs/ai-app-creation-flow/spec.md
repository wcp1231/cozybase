...
定义用户通过自然语言创建 APP 的完整链路，包括信息提取、slug 生成、APP 初始化、后台启动 Agent，以及前端跳转后的会话衔接。
## Requirements
...
- **THEN** chat session SHALL 通过 `session.history` 推送 Agent 已产生的消息
...
### Requirement: APP displayName 可单独更新

系统 SHALL 提供独立的 APP metadata 更新能力，允许前端只更新 `display_name` 而不修改 slug、URL、目录或运行时标识。Draft APP 页面标题旁 SHALL 提供修改显示名称的入口。

#### Scenario: 仅更新 display_name

- **WHEN** 客户端调用 APP metadata 更新接口，仅提交新的 `display_name`
- **THEN** 系统 SHALL 更新该 APP 的 `display_name`
- **AND** 系统 SHALL 保持 slug、URL 路由和数据目录不变

#### Scenario: Draft APP 页面修改显示名称

- **WHEN** 用户在 Draft APP 页面点击标题旁的修改入口并保存新的显示名称
- **THEN** 页面标题、APP 列表和其他可见名称展示 SHALL 使用更新后的 `displayName`
- **AND** 若新的显示名称为空字符串，UI SHALL 回退显示 slug
