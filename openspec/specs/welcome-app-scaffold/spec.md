...
系统 SHALL 在 `packages/server/templates/welcome/` 目录下维护 Welcome 应用模板，目录结构与实际 app 目录完全一致：
...
- **WHEN** 开发者检查 `packages/server/templates/welcome/` 目录
...
- **WHEN** 客户端发送 `GET /draft/apps/welcome/functions/todos`
- **THEN** 系统 SHALL 返回 `todo` 表中的所有记录

#### Scenario: 按状态筛选 — 已完成

- **WHEN** 客户端发送 `GET /draft/apps/welcome/functions/todos?status=completed`
- **THEN** 系统 SHALL 仅返回 `completed = 1` 的记录

#### Scenario: 按状态筛选 — 未完成

- **WHEN** 客户端发送 `GET /draft/apps/welcome/functions/todos?status=pending`
...
- **WHEN** 客户端发送 `POST /draft/apps/welcome/functions/todos`，body 为 `{ "title": "买牛奶" }`
- **THEN** 系统 SHALL 在 `todo` 表中插入一条新记录，并返回包含 `id`、`title`、`completed`、`created_at` 的完整记录

#### Scenario: 缺少 title 字段

- **WHEN** 客户端发送 `POST /draft/apps/welcome/functions/todos`，body 为 `{}` 或 `{ "title": "" }`
...
- **WHEN** 客户端发送 `DELETE /draft/apps/welcome/functions/todos`，body 为 `{ "id": 1 }`，且 `id=1` 的记录存在
- **THEN** 系统 SHALL 从 `todo` 表中删除该记录，并返回删除成功的确认信息

#### Scenario: 删除不存在的记录

- **WHEN** 客户端发送 `DELETE /draft/apps/welcome/functions/todos`，body 为 `{ "id": 999 }`，且该记录不存在
- **THEN** 系统 SHALL 返回 404 错误

#### Scenario: 缺少 id 字段

- **WHEN** 客户端发送 `DELETE /draft/apps/welcome/functions/todos`，body 为 `{}`
