...
Action 的 `url` 字段 SHALL 为 App 相对路径（如 `/db/todo`、`/functions/todos`），渲染器根据当前 App 的 `baseUrl` 自动补全为完整路径。
...
- **WHEN** 按钮点击触发 `[{ "type": "api", ... }, { "type": "reload", "target": "todo-table" }]`
- **THEN** 系统 SHALL 先执行 api 请求，成功后再执行 reload

#### Scenario: api action 的 onSuccess 回调

- **WHEN** api action 包含 `"onSuccess": [{ "type": "reload", "target": "todo-table" }, { "type": "close" }]`
- **THEN** 系统 SHALL 在 API 请求成功后依次执行 reload 和 close action

#### Scenario: api action 的 onError 回调

- **WHEN** api action 包含 `"onError": [{ "type": "alert", "message": "操作失败" }]`，且 API 请求返回错误
- **THEN** 系统 SHALL 执行 onError 中定义的 action

#### Scenario: confirm action

- **WHEN** 按钮点击触发 `{ "type": "confirm", "message": "确认删除？", "onConfirm": { "type": "api", ... } }`
- **THEN** 系统 SHALL 弹出确认对话框，用户确认后执行 onConfirm 中的 action

#### Scenario: dialog action

- **WHEN** 按钮点击触发 `{ "type": "dialog", "title": "新建", "body": { "type": "form", ... } }`
- **THEN** 系统 SHALL 打开一个标题为"新建"的弹窗，弹窗内渲染 body 中的组件 schema
...
- **WHEN** 组件 visible 属性为 `"${row.completed === 0}"`
- **THEN** 系统 SHALL 将其求值为 boolean 结果（`true` 或 `false`）

#### Scenario: 三元表达式

- **WHEN** 组件 text 属性为 `"${row.completed === 1 ? '已完成' : '待完成'}"`
- **THEN** 系统 SHALL 根据 `row.completed` 的值返回对应的字符串
