...
MCP 工具集 SHALL 支持文件系统同步工作流，替代原有的 Checkout-Edit-Push 模式。
...
**Reason**: 替换为文件系统同步工作流。新工作流中 Agent 在本地文件系统读写文件，MCP 工具负责同步，取代了通过 MCP 参数传递文件内容的模式。
**Migration**: 使用新的"文件系统同步工作流"（ADDED Requirements 中定义）替代。
