...
11. 返回执行结果（成功/失败 + 已执行的 migration 列表 + 函数验证结果）
...
- **WHEN** Agent 调用 `POST /draft/apps/todo-app/reconcile`，`app_files` 中有 3 个 migration 记录和 1 个 seed 记录
- **THEN** 系统 SHALL 销毁旧的 draft.sqlite，创建新库，按序执行 3 个 migration，加载 seed，导出 function 文件并验证，返回成功结果
...
10. 清理 `draft/apps/{appName}/db.sqlite`（删除 draft 数据库）
11. 返回发布结果
...
- **THEN** 系统 SHALL 备份 stable.sqlite，增量执行新 migration，更新 _migrations 表，标记 migration 为 immutable，更新 published_version，导出 function 到 stable 目录，通知 FunctionRuntime reload，清理 draft.sqlite

#### Scenario: 新 App 的首次 Publish

- **WHEN** Agent 对 Draft only 状态的 App 调用 Publish
- **THEN** 系统 SHALL 创建 `data/apps/{appName}/db.sqlite`，执行全部 migration，记录 _migrations，标记 migration 为 immutable，设置 published_version = current_version，导出 function 文件，通知 FunctionRuntime reload，清理 draft.sqlite

#### Scenario: Publish 失败并回滚

- **WHEN** Publish 过程中 migration 执行失败
- **THEN** 系统 SHALL 用备份文件恢复 stable.sqlite，返回错误信息，不更新 published_version，不标记 immutable，不通知 FunctionRuntime reload
