...
#### Scenario: bun install 失败不阻断主流程

- **WHEN** `bun install` 因网络错误执行失败
- **THEN** reconcile/publish 整体结果 SHALL 仍为成功，返回结果中 SHALL 包含 npm 安装警告信息，函数文件和数据库迁移不受影响
...
#### Scenario: 函数 import 第三方包

- **WHEN** `stable/{appName}/functions/hello.ts` 中包含 `import dayjs from 'dayjs'`，且 `stable/{appName}/node_modules/dayjs` 已安装
- **THEN** Runtime 动态 import 该函数时，Bun SHALL 成功解析 `dayjs`，函数正常执行

#### Scenario: 函数 import 未安装的包

- **WHEN** 函数代码 import 了未在 `package.json` 中声明的包
- **THEN** Runtime 动态 import 时 SHALL 抛出模块未找到错误，以运行时错误形式返回给调用方
