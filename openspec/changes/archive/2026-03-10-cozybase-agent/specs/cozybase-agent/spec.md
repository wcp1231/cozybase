...
CozyBase Agent 的会话实现（`CozyBaseSession`）SHALL 是独立的 session 类，MUST NOT 继承 `RuntimeAgentSession`。CozyBaseSession SHALL 直接使用 `packages/ai-runtime` 的 `AgentRuntimeProvider.createQuery()` 创建 agent 查询，自行管理 session 生命周期。CozyBaseSession MUST NOT 持有 `appSlug` 属性，MUST NOT 注册 per-app 的 MCP server。
...
CozyBase Agent SHALL 拥有以下直接工具，通过 daemon 内部 API（AppManager 等）执行。每个工具 SHALL 定义标准的 MCP tool schema。
...
- **AND** SHALL 将构建任务入队 TaskRegistry（target 为 `builder`）
...
- **THEN** 工具 SHALL 将开发任务入队 TaskRegistry（target 为 `builder`）
- **AND** SHALL 立即返回 `{ taskId: string, appSlug: string, status: "queued" | "running" }`

#### Scenario: operate_app 异步委派数据操作

- **WHEN** Agent 调用 `operate_app({ app_name: "ledger", instruction: "查询本月总支出" })`
- **THEN** 工具 SHALL 将操作任务入队 TaskRegistry（target 为 `operator`）
- **AND** SHALL 立即返回 `{ taskId: string, appSlug: string, status: "queued" | "running" }`
...
系统 SHALL 提供 `TaskRegistry`，管理 CozyBase Agent 发起的异步委派任务。TaskRegistry SHALL 维护 per-app、per-target 的任务队列，以 `"{appSlug}:{target}"` 作为队列 key。同一队列内的任务 SHALL 串行执行。不同队列（不同 APP 或不同 target 类型）的任务 SHALL 可以并行。

TaskRegistry SHALL 在内存中维护，daemon 重启后任务状态丢失。
...
- **WHEN** CozyBaseSession 正在处理用户 prompt（streaming 状态）
...
- `cozybase_agent.agent_provider`：runtime provider
- `cozybase_agent.model_provider`：底层模型厂商（仅 `pi-agent-core` 时使用）
- `cozybase_agent.model`：具体模型
...
- **AND** 无相关环境变量
- **THEN** 系统 SHALL 使用默认值：`agent_provider=claude-code`，`model=claude-haiku`

### Requirement: 消息历史独立持久化

CozyBaseSession 的消息历史 SHALL 独立持久化到 `RuntimeSessionStore`，使用 `usage_type` 为 `'cozybase'`、`app_slug` 为固定值 `'__cozybase__'`。该历史 MUST NOT 与任何 per-app Builder/Operator session 共享。

#### Scenario: 消息持久化到独立的存储空间

- **WHEN** CozyBase Agent 完成一轮对话（用户消息 + Agent 回复）
