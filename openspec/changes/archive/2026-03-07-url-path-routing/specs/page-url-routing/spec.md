...
系统 SHALL 将路径匹配提取的参数（如 `{ orderId: "1024" }`）与 URL query string 参数合并后传入 `SchemaRenderer` 的 `params` 上下文。路径参数 MUST 优先于同名 query 参数。

#### Scenario: 路径参数在表达式中可用

- **WHEN** 页面中某组件使用表达式 `${params.orderId}`
- **AND** 当前 URL 子路径为 `orders/1024`（匹配 `orders/:orderId`）
- **THEN** 表达式 SHALL 解析为 `"1024"`

#### Scenario: 路径参数覆盖同名 query 参数

- **WHEN** URL 子路径为 `orders/1024`（提取 `orderId: "1024"`）
- **AND** URL query string 包含 `?orderId=999`
- **THEN** `params.orderId` SHALL 为 `"1024"`（路径参数优先）

### Requirement: Tab 栏仅显示顶层页面

系统 SHALL 将路径不包含 `/` 分隔符的页面识别为顶层页面。Tab 栏 MUST 只渲染顶层页面，子页面（路径包含 `/`）MUST NOT 出现在 Tab 栏中。

#### Scenario: 顶层页面显示在 Tab 栏

- **WHEN** APP 的 `pages[]` 包含 `path: "orders"` 和 `path: "orders/:orderId"`
- **THEN** Tab 栏 SHALL 只显示 `orders` 页面
- **AND** `orders/:orderId` MUST NOT 出现在 Tab 栏中

#### Scenario: 无子路径时重定向到第一个顶层页面

- **WHEN** 用户访问 APP 根路径（无子路径）
...
- **WHEN** 面包屑显示多级路径
- **THEN** 除最后一级（当前页面）外，其余面包屑节点 SHALL 可点击跳转到对应路径

#### Scenario: 面包屑跳过无页面定义的中间层级

- **WHEN** URL 子路径为 `orders/1024/refund`
- **AND** `pages[]` 中不存在 `orders/:orderId` 页面（只有 `orders` 和 `orders/:orderId/refund`）
- **THEN** 面包屑 SHALL 渲染为 `APP名 / 订单列表 / 申请退款`（跳过中间层级）
